...
@app.route("/fluent-bit", methods=["POST"])
def receive_fluent_bit():
    """
    Accepts the array payload that Fluent Bit's http output plugin sends
    when Format=json.  Each element has at minimum a "log" key (the JSON
    string serialised by wrap.lua) plus "tag", vendor/dev_type metadata, etc.

    We extract the "log" value from every element; if it is already a string
    we keep it as-is, if it is a dict (Fluent Bit sometimes inlines parsed
    JSON) we re-serialise it.
    """
    payload = request.get_json(force=True, silent=True)
    if not payload:
        return jsonify({"error": "invalid JSON body"}), 400

    # Fluent Bit sends either a bare array or {"date": ..., "records": [...]}
    # depending on the plugin version — handle both shapes.
    ...
    for item in items:
        ...
        log_field = item.get("log")
        if log_field is None:
            # No "log" key — serialise the whole item so nothing is dropped
            records.append(json.dumps(item))
        elif isinstance(log_field, str):
            records.append(log_field)
        else:
            # wrap.lua already serialised it, but just in case it's a dict
            records.append(json.dumps(log_field))
    ...
    logger.info("[PRODUCER] Fluent Bit: buffered %d record(s). Buffer size: %d",
                len(records), len(buffer))
    return jsonify({"status": "ok", "accepted": len(records)}), 200
