...
- For "timestamp": if the log contains an ISO-8601 timestamp use it; otherwise use
  the "ingestion_time" field (already converted to ISO-8601) provided in the log object.
  Never use "unknown" for timestamp if ingestion_time is present.
...
def start_node(state: PipelineState) -> PipelineState:
    ...
    # Enrich each raw log string with an ingestion_time field so the LLM
    # can use it as a fallback timestamp when the device log has none.
    import datetime
    enriched_batch = []
    for log in batch:
        try:
            ...
            log_obj = {"raw": log}
        if "ingestion_time" not in log_obj:
            # Fluent Bit stamps each record with a Unix epoch "date" field
            epoch = log_obj.get("date")
            if epoch:
                iso = datetime.datetime.utcfromtimestamp(float(epoch)).strftime(
                    "%Y-%m-%dT%H:%M:%SZ"
                )
            else:
                iso = datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
            log_obj["ingestion_time"] = iso
        enriched_batch.append(json.dumps(log_obj))

    return {
        **state,
        "current_batch":    batch,
        "pending_raw_logs": enriched_batch,
